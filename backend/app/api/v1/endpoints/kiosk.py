...
GET  /api/v1/kiosk/queue
    Returns active OPD tokens sorted: RED-flag first, then FIFO.

POST /api/v1/kiosk/queue/{token_id}/status
    Update a token's status (IN_CONSULTATION | COMPLETED).
...
def kiosk_intake(
    ...
    # Assign cabin deterministically from complaint key for display purposes
    _CABIN_MAP = {
...
@router.get("/queue")
def get_opd_queue() -> Dict[str, Any]:
    """
    Return active OPD queue.
    Sort order: RED-flag WAITING tokens first (urgent), then by created_at FIFO.
    Excludes COMPLETED tokens.
    """
    active = [
        entry for entry in OPD_QUEUE.values()
        if entry["status"] != "COMPLETED"
    ]
    # Sort: red+waiting → red+in_consultation → green+waiting → green+in_consultation
    def sort_key(e: Dict[str, Any]) -> tuple:
        red = 0 if e["red_flag"] else 1
        status_order = {"WAITING": 0, "IN_CONSULTATION": 1}.get(e["status"], 2)
        return (red, status_order, e["created_at"])

    active.sort(key=sort_key)
    return {"queue": active, "total": len(active)}


class QueueStatusUpdate(BaseModel):
    status: Literal["IN_CONSULTATION", "COMPLETED"]


@router.post("/queue/{token_id}/status")
def update_queue_status(token_id: str, body: QueueStatusUpdate) -> Dict[str, Any]:
    """Update a queue entry's status to IN_CONSULTATION or COMPLETED."""
