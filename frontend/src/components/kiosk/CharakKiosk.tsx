...
const AbhaQrModal: React.FC<AbhaQrModalProps> = ({ dark, onClose, onScan }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
      <div className={`w-full max-w-sm max-h-[calc(100dvh-2rem)] overflow-y-auto rounded-2xl border shadow-2xl p-4 sm:p-5 space-y-4 ${bg}`}>
        ...
        <div className="space-y-2">
          ...
          <div className="flex gap-2">
            <input
              ...
              className={`flex-1 min-w-0 px-3 py-2 rounded-xl border text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 ${inp}`}
            />
            <input
              placeholder="Age"
              type="number"
...
export const CharakKiosk: React.FC<CharakKioskProps> = ({ onRedFlag }) => {
        ...
        <div className={`rounded-2xl border-2 border-dashed p-3 space-y-2 ${
          ...
          <div className="flex flex-col sm:flex-row sm:items-center gap-2">
            <button
              ...
              className={`flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl font-bold text-sm transition-all shrink-0 ${
            ...
            <div className="flex flex-1 min-w-0 gap-1.5">
              <input
                ...
                className={`flex-1 min-w-0 px-3 py-2.5 sm:py-2 rounded-xl border text-xs focus:outline-none focus:ring-2 ${th.input}`}
              />
              <button
          ...
          {abhaId && (
            <span className={`flex items-center gap-1.5 text-xs font-semibold break-all ${dark ? 'text-emerald-300' : 'text-emerald-700'}`}>
              <CheckCircle2 className="w-3.5 h-3.5 shrink-0" /> ABHA Verified: {abhaId}
            </span>
          )}
        </div>
          ...
          <div className={`rounded-2xl border-2 p-4 space-y-3 animate-fade-in ${
            ...
            <div className="grid grid-cols-2 gap-x-4 gap-y-1.5 text-xs">
              ...
              <div>
                ...
                <p className={`font-mono font-bold text-emerald-600 dark:text-emerald-300 break-all`}>{abhaPreview.abhaId}</p>
              </div>
              {abhaPreview.age && (
                <div>
            ...
            {AGE_GROUPS.map((ag) => {
              ...
              return (
                <button
                  ...
                  className={`flex flex-col items-center gap-1.5 py-3 px-1 sm:px-2 rounded-2xl border-2 text-center transition-all ${
                    sel
            ...
            {([
              ...
              return (
                <button
                  ...
                  className={`flex flex-col items-center gap-2 py-3 sm:py-4 px-1.5 sm:px-2 rounded-2xl border-2 transition-all ${
                    isSelected
  ...
  const renderStep1 = () => (
    <div className="space-y-6">
      ...
      {/* ── Hero Voice Orb ─────────────────────────────────────────────────── */}
      <div className={`rounded-3xl border-2 p-4 sm:p-5 flex flex-col items-center gap-4 overflow-hidden ${
        isListening
  ...
  const renderStep2 = () => (
    <div className="space-y-7">
        ...
        <div className="p-4">
          {vitalsScan === 'idle' && (
            <div className="flex flex-col items-center gap-3 py-4">
              ...
              <p className={`text-sm font-semibold text-center ${th.subtext}`}>Place finger on sensor pad, then tap Start Scan</p>
          ...
          {vitalsScan === 'scanning' && (
            <div className="flex flex-col items-center gap-4 py-4">
              ...
              <div className="flex gap-2 sm:gap-4 text-center">
                {['BP','SpO₂','Pulse','Temp'].map((v) => (
                  <div key={v} className={`rounded-xl px-2.5 sm:px-3 py-2 min-w-[52px] ${dark ? 'bg-slate-800' : 'bg-white border border-slate-200'}`}>
            ...
            <div className="space-y-3">
              <div className="grid grid-cols-2 gap-2">
                ...
                <div className={`rounded-xl border p-3 ${dark ? 'border-emerald-700/50 bg-slate-900' : 'border-emerald-300 bg-white'}`}>
                  ...
                  <p className={`text-lg sm:text-xl font-black tabular-nums ${dark ? 'text-emerald-300' : 'text-emerald-700'}`}>
                ...
                <div className={`rounded-xl border p-3 ${dark ? 'border-cyan-700/50 bg-slate-900' : 'border-cyan-300 bg-white'}`}>
                  <p className={`text-[9px] uppercase font-bold mb-0.5 ${dark ? 'text-slate-500' : 'text-slate-400'}`}>SpO₂</p>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
          {AGNI_OPTIONS.map((opt) => (
            <button
              ...
              className={`p-3 sm:p-4 rounded-2xl border-2 flex flex-col items-center gap-2 text-center transition-all ${
                agni === opt.id
        ...
        <div className="grid grid-cols-3 gap-2 sm:gap-3">
          {KOSHTHA_OPTIONS.map((opt) => (
            <button
              ...
              className={`p-2.5 sm:p-4 rounded-2xl border-2 flex flex-col items-center gap-2 text-center transition-all ${
                koshtha === opt.id
                  ? (dark ? opt.active_dark : opt.active_light)
                  : (dark ? opt.inactive_dark : opt.inactive_light)
              }`}
      ...
      {/* Pain VAS Slider */}
      <div>
        <p className={`text-sm font-bold mb-3 flex flex-wrap items-center gap-x-2 gap-y-1 ${dark ? 'text-slate-300' : 'text-slate-700'}`}>
      ...
      <div>
        ...
        <div className="flex gap-2">
          <input value={alloInput} onChange={(e) => setAlloInput(e.target.value)}
            ...
            className={`flex-1 min-w-0 px-3 py-2 rounded-xl border text-sm focus:outline-none focus:ring-2 ${th.input}`}
          />
    ...
    return (
      <div className="space-y-5">
        ...
        <div className="flex flex-col items-center gap-4 py-4">
          ...
          <div className={`px-6 sm:px-10 py-6 rounded-2xl border-2 text-center shadow-lg w-full max-w-xs ${th.tokenCard(isRed)}`}>
            <p className={`text-xs font-semibold uppercase tracking-widest mb-1 ${th.tokenSub}`}>OPD Token</p>
            <p className={`text-4xl sm:text-5xl font-black tracking-wider break-all ${th.tokenNum(isRed)}`}>{result.token_id}</p>
          ...
          <div className="w-full grid grid-cols-2 gap-2 sm:gap-3">
            <div className={`rounded-xl border px-2 sm:px-4 py-3 text-center ${dark ? 'bg-slate-800 border-slate-700' : 'bg-slate-50 border-slate-200'}`}>
    ...
    <div className="flex items-center justify-center gap-1.5 sm:gap-2 mb-6">
      {[0, 1, 2, 3].map((s) => (
        <React.Fragment key={s}>
          ...
          {s < 3 && (
            <div className={`h-0.5 w-6 sm:w-8 rounded transition-all ${s < step ? th.dotLineDone : th.dotLine}`} />
          )}
        </React.Fragment>
      ))}
    </div>
  ...
  return (
    <div className={`min-h-screen flex flex-col transition-colors duration-200 ${th.screen}`}>
      ...
      {/* ── Kiosk Top Bar ──────────────────────────────────────────────────── */}
      <header className={`sticky top-0 z-40 shrink-0 border-b px-3 sm:px-5 py-3 flex items-center justify-between gap-2 sm:gap-4 ${th.header}`}>
        {/* Brand */}
        <div className="flex items-center gap-2.5 min-w-0">
          ...
          <div className="flex flex-col leading-tight min-w-0">
            ...
            <span className={`text-[10px] font-medium truncate hidden sm:block ${th.subtext}`}>
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
          <div className="hidden sm:block"><LiveClock dark={dark} /></div>
          ...
          <div className="relative">
            ...
            {showLangMenu && (
              <div
                className={`absolute right-0 top-full mt-1.5 z-50 rounded-2xl border shadow-2xl p-2 grid grid-cols-2 sm:grid-cols-3 gap-1 min-w-[220px] max-w-[calc(100vw-1.5rem)] ${
      ...
      <main className="flex-1 overflow-y-auto pb-20">
        <div className="max-w-2xl mx-auto px-3 sm:px-4 py-4 sm:py-6">
          <div className={`rounded-2xl border shadow-xl p-4 sm:p-6 ${th.card}`}>
            {step < 4 && <StepDots />}
              ...
              <div className="flex gap-2 sm:gap-3 mt-6 sm:mt-8">
                {step > 0 && (
                  <button
                    ...
                    className={`flex items-center gap-2 px-4 sm:px-5 py-3 rounded-xl border font-semibold text-sm transition-colors ${th.backBtn}`}
                  >
